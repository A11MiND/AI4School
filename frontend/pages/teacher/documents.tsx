...
export default function ContentLibrary() {
    ...
    return (
        <div className="space-y-6">
             ...
             <div className="flex gap-4 items-center bg-white p-3 rounded-xl border border-slate-200 shadow-sm overflow-x-auto">
                {breadcrumbs.map((crumb, idx) => (
                    <div key={idx} className="flex items-center gap-2 whitespace-nowrap">
                        {idx > 0 && <ChevronRight size={16} className="text-slate-400" />}
                        <button 
                            onClick={() => navigateToBreadcrumb(idx)}
                            className={`flex items-center gap-1 text-sm font-medium ${idx === breadcrumbs.length - 1 ? 'text-indigo-600' : 'text-slate-600 hover:text-slate-900'}`}
                        >
                            {idx === 0 && <Home size={14} />}
                            {crumb.name}
                        </button>
             ...
             <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                 ...
                 <div className="lg:col-span-1 space-y-4">
                        ...
                        {showCreateFolder && (
                            <div className="mt-3 space-y-2">
                                ...
                                <button 
                                    ...
                                    className="w-full py-2 bg-indigo-600 text-white rounded-lg text-xs font-semibold hover:bg-indigo-700"
                                >
                                    Create
                                </button>
                            </div>
                    ...
                    <div className="bg-white p-6 rounded-xl border border-dashed border-slate-300 text-center hover:bg-slate-50 transition-colors flex flex-col justify-center">
                        <div className="mx-auto flex justify-center text-indigo-500 mb-4 bg-indigo-50 p-3 rounded-full">
                            <Upload size={32} />
                        </div>
                        ...
                        <div className="w-full flex flex-col gap-3">
                            ...
                            <button 
                                ...
                                className="py-2.5 px-4 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 disabled:opacity-50 flex items-center justify-center gap-2 transition-all shadow-sm"
                            >
                 ...
                 <div className="lg:col-span-3 bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden min-h-[500px]">
                     {documents.length === 0 ? (
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
                            {documents.map((doc) => (
                                <div key={doc.id} className="group relative bg-white border border-slate-200 rounded-xl p-4 hover:shadow-md transition-shadow cursor-default flex flex-col justify-between h-48">
                                    ...
                                    <div className="flex justify-center gap-4 mt-2 pt-3 border-t border-slate-100 opacity-0 group-hover:opacity-100 transition-opacity">
                                       {!doc.is_folder && (
                                            <button 
                                                title="Create Exam"
